...
def plot(
    ...
    for param in param_dict.keys():
        ...
        # Y Axis
        if param == 'soot_mass':
            ax.set_ylabel(r'Soot mass $\mathregular{[\mu g/g]}$', {'fontsize' : AXIS_FONTSIZE})
            ax.set_ylim([0, 210]);
        elif param == 'N_agg':
            ax.set_yscale('log');
            ax.set_ylabel(r'$\mathregular{N_{{agg}} [1/g]}$', {'fontsize' : AXIS_FONTSIZE})
            ax.set_ylim([1e11, 3e14]);
        elif param == 'd_p':
            ax.set_ylabel(r'Primary Particle Diameter $\mathregular{[nm]}$', {'fontsize' : AXIS_FONTSIZE})
            ax.set_ylim([1, 15]);
        elif param == 'd_m':
            ax.set_ylabel(r'Mobility Diameter $\mathregular{[nm]}$', {'fontsize' : AXIS_FONTSIZE})
            ax.set_ylim([2, 80]);
        _save_plot(param)